...
spool /accesounix/datos/pptos_ods.csv append
...
declare
  ...
  is
    select   opa.id_claso id_claso,
			 ...
			 vaoe.id_actividad_objetivo_pad objetivo,
             substr(vaoe.id_actividad_objetivo, instr(vaoe.id_actividad_objetivo, '.')+1, length(vaoe.id_actividad_objetivo)) meta,
             ' ' sinergia,
             opa.vi_org vi_org,
             opa.vi_fun vi_fun,
             opa.vi_eco vi_eco,
        ...
        from ns_vog_parti_gcp opa,
             ns_vca_act_obj_enlaces vaoe
       where opa.oee_emp_cdi_empresa = argu_cdi_empresa
         and opa.oee_cdi_eje = argu_cdi_eje
		 and nvl (vaoe.oet_cdi_emptgen_ambito, argu_cdi_ambito) = argu_cdi_ambito
         and nvl (vaoe.oet_cdi_emptgen_tipo, argu_cdi_tipo) = argu_cdi_tipo
		 and opa.cdi_par = vaoe.opa_cdi_par
         and vaoe.rfao_type = 'OPA'
         and nvl(v_todos,  ns_cana_1.lee_datos_sobre_act_obj(argu_cdi_empresa, 
                                           argu_cdi_eje, 
                                           'CUALES_METAS_HAY_EN_CDI_PAR', 
                                           v_cdi_ambito, 
                                           v_cdi_tipo, 
                                           opa.cdi_par, 
                                           ',', 
